...
const CustomLink = ({ href, title, className = "" }) => {
  ...
  return (
    <Link href={href} className={`${className} relative group`}>
      ...
      {router.asPath === href && (
        <span
          className={`h-[1px] inline-block bg-dark absolute left-0 -bottom-0.5 w-full transition-[width] ease duration-300`}
        />
      )}
    </Link>
  );
...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
  ...
  return (
    ...
    >
      ...
      {router.asPath === href && (
        <span
          className={`h-[1px] inline-block bg-light absolute left-0 -bottom-0.5 w-full transition-[width] ease duration-300
          }`}
        />
      )}
    </button>
    ...
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between relative z-10 lg:px-16 md:px-12 sm:px-8">
      ...
      <div className="w-full flex justify-between items-center lg:hidden">
        ...
        <nav className="flex items-center justify-center flex-wrap">
          <motion.a
            href="https://www.linkedin.com/in/zi-ying-heng-7444aa249/"
            target={"_blank"}
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.9 }}
            className="w-6 mr-3"
          ...
          <motion.a
            href="https://github.com/hengziying/Projects"
            target={"_blank"}
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.9 }}
            className="w-6 mx-3"
